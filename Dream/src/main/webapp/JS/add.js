...
function initialization(){
    scripForm.wishMan.value="";												//清空祝福对象文本框
    scripForm.wellWisher.value="匿名";										//初始化祝福者文本框为“匿名”
    scripForm.color.value=0;												//初始化字条颜色
    scripForm.face.value=0;													//初始化表情图案
    scripForm.content.value="";												//初始化字条内容
    scripForm.used.value=0;													//初始化已用字节数
    scripForm.remain.value=200;												//初始化剩余字节数
    /************************初始化验证码*******************************************************/
...
function scripSubmit(){
    ...
    var param="wishMan="+scripForm.wishMan.value+"&wellWisher="+scripForm.wellWisher.value+"&color="+scripForm.color.value+"&face="+scripForm.face.value+"&content="+			scripForm.content.value; 	//将许愿字条的全部内容连接成字符串，作为发送请求的参数
