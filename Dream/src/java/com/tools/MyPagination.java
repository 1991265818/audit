...
public class MyPagination {
	public List<ScripForm> list=null;
	private int recordCount=0;
	private int pagesize=0;
	private int maxPage=0;

	//初始化分页信息
	public List<ScripForm> getInitPage(List<ScripForm> list,int Page,int pagesize){
		List<ScripForm> newList=new ArrayList<ScripForm>();
		this.list=list;
		...
		try{
			for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
				...
				newList.add((ScripForm)list.get(i));
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	//获取指定页的数据
	public List<ScripForm> getAppointPage(int Page){
		List<ScripForm> newList=new ArrayList<ScripForm>();
		try{
			for(int i=(Page-1)*pagesize;i<=Page*pagesize-1;i++){
				try{
